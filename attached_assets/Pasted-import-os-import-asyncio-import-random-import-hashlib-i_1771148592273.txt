...
"""
    ...
    async def matches_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        matches = self.db.get_user_matches(user_id, limit=15)
        
        if not matches:
            await update.message.reply_text("📋 No match history found.")
            return
        
        text = f"📋 <b>Match History - {username}</b>\n\n"
        for m in matches:
            game_type = m.get('type', 'unknown')
            # Clean up game type display - remove underscores, title case
            ...
            if result == 'win' or winner == user_id:
                result_emoji = "✅"
                result_text = f"+${payout:,.2f}"
            elif result == 'loss' or (winner is not None and winner != user_id):
                result_emoji = "❌"
                result_text = f"-${wager:,.2f}"
            else:
                result_emoji = "➖"
                result_text = f"${payout:,.2f}" if payout else "Push"
            ...
            text += f"{result_emoji} {game_type} | ${wager:,.2f} | {result_text}"
            if date_str:
                text += f" | {date_str}"
            text += "\n"
...
"""
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Blackjack bet change (Half/Double buttons on game-over screen)
        if data.startswith("bj_bet_change_"):
            ...
            # Rebuild the same game-over message but with updated bet
            keyboard = [
        ...
        if data == "menu_more":
            keyboard = [
                [
                    InlineKeyboardButton("📊 Your Statistics", callback_data="menu_stats"),
                    InlineKeyboardButton("📅 Matches History", callback_data="menu_matches")
                ],
        ...
        if data == "menu_stats":
            ...
            elif total_wagered >= 75000: rank_emoji = "🥈 Silver V"
            elif total_wagered >= 50000: rank_emoji = "🥈 Silver IV"
            elif total_wagered >= 25000: rank_emoji = "🥈 Silver III"
            elif total_wagered >= 15000: rank_emoji = "🥈 Silver II"
            elif total_wagered >= 10000: rank_emoji = "🥈 Silver I"
            elif total_wagered >= 7500: rank_emoji = "🥉 Bronze V"
            elif total_wagered >= 5000: rank_emoji = "🥉 Bronze IV"
            elif total_wagered >= 2500: rank_emoji = "🥉 Bronze III"
            elif total_wagered >= 1000: rank_emoji = "🥉 Bronze II"
            else: rank_emoji = "🥉 Bronze I"
            ...
            stats_text = (
                ...
                f"First game: <b>{first_game_str}</b>\n"
                f"Last game: <b>{last_game_str}</b>"
            )
